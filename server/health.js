	...
	fetch('http://next-flags.ft.com/')
		...
		.then(flags => {
			return Promise.all(flags
				...
				.map(f => fetch(`https://www.hostedgraphite.com/bbaf3ccf/${process.env.HOSTEDGRAPHITE_READ_APIKEY}/graphite/render/?_salt=1445269005.584&from=-10minutes&target=divideSeries(summarize(heroku.ab.*.allocation.${f.name}.on%2C%2210min%22)%2Csummarize(sumSeries(heroku.ab.*.allocation.${f.name}.*)%2C%2210min%22))&format=json`)
										.then(fetchres.json)
										.then(data => {
											return {
												name: f.name,
